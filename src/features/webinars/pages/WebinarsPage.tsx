...
export const WebinarsPage: React.FC = () => {
  ...
  const columns: ColumnDef<Webinar>[] = [
        ...
        return (
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-muted-foreground line-clamp-1 text-xs">{w.description}</p>
            </div>
          </div>
        );
      },
      ...
      cell: ({ row }) => {
        ...
        return (
          <div>
            ...
            <p className="text-muted-foreground text-[10px] leading-none">{w.speakerDesignation}</p>
          </div>
        );
      },
    },
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        else if (status === 'Archived') color = 'bg-amber-500/10 text-amber-500';
        return (
          ...
          <div className="flex gap-2">
            ...
            {w.status !== WebinarStatus.Cancelled && (
              <Button
                variant="ghost"
                size="icon"
                onClick={() => handleStatusChange(w.id, WebinarStatus.Cancelled)}
  ...
  return (
    <div className="animate-fade-in space-y-6 p-6">
      ...
      <div className="flex flex-col items-start justify-between gap-4 sm:flex-row sm:items-center">
        <div>
          ...
          <p className="text-muted-foreground mt-1 text-sm font-medium">
            Publish educational parent seminars, edit speaker bios, copy meeting links, and monitor
            registrant rosters.
          </p>
        </div>
      </div>
      ...
      <Dialog open={isCreateOpen} onOpenChange={setIsCreateOpen}>
        <DialogContent className="bg-card border-border !max-w-3xl rounded-2xl p-0 shadow-2xl">
          <DialogHeader className="px-6 pt-6">
            <DialogTitle className="text-xl font-bold">Schedule Webinar Seminar</DialogTitle>
            <DialogDescription>
              Organize a new online presentation or parenting workbook review session.
            </DialogDescription>
          </DialogHeader>
      ...
      {/* Edit Dialog */}
      <Dialog open={!!editingWebinar} onOpenChange={(open) => !open && setEditingWebinar(null)}>
        <DialogContent className="bg-card border-border rounded-2xl shadow-2xl">
          <DialogHeader>
            <DialogTitle className="text-xl font-bold">Edit Webinar details</DialogTitle>
            <DialogDescription>Modify parameters for: {editingWebinar?.title}.</DialogDescription>
          </DialogHeader>
          {editingWebinar && (
            <CreateWebinarForm
      ...
      <Dialog open={!!selectedWebinar} onOpenChange={(open) => !open && setSelectedWebinar(null)}>
        <DialogContent className="bg-card border-border max-w-md rounded-2xl shadow-2xl">
          {selectedWebinar && (
            <WebinarRosterList webinar={selectedWebinar} onClose={() => setSelectedWebinar(null)} />
          )}
        </DialogContent>
      </Dialog>
    </div>
...
const WebinarRosterList: React.FC<{
  ...
  return (
    <div className="space-y-4 pt-2">
      <DialogHeader>
        <DialogTitle className="text-lg font-bold">Webinar Attendee Roster</DialogTitle>
        <DialogDescription>
