...
export const CommunitiesPage: React.FC = () => {
  ...
  const [groupCat, setGroupCat] = useState('Neighborhood');
  ...
  const columns: ColumnDef<ICommunity>[] = [
...

    {
      ...
      cell: ({ row }) => {
        ...
        return (
          <span className={`rounded-md px-2 py-1 text-xs font-medium capitalize ${colors[status]}`}>
        ...
        return (
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end" className="w-52">
              ...
              <DropdownMenuItem asChild>
                <Link href={`/admin/communities/${community.id}/messages`}>
              ...
              <DropdownMenuItem asChild>
                <Link href={`/admin/communities/${community.id}/analytics`}>
  ...
  console.log('CommunitiesPage data:', data);
  console.log(data);
  console.log(columns);
  ...
  return (
    <div className="animate-fade-in space-y-6 p-6">
      ...
      <div className="flex flex-col items-start justify-between gap-4 sm:flex-row sm:items-center">
        <div>
          <h1 className="text-3xl font-extrabold tracking-tight">Community Network</h1>
          <p className="text-muted-foreground mt-1 text-sm font-medium">
            Oversee community neighborhood hubs, verify communities data, and moderate group
            announcements.
          </p>
        </div>
      </div>
      ...
      <DataTable
        ...
        onRowClick={(cmty) => router.push(`/communities/${cmty.id}`)}
        toolbar={
          <Button
      ...
      <Dialog open={isGroupOpen} onOpenChange={setIsGroupOpen}>
        <DialogContent className="bg-card border-border max-w-md rounded-2xl shadow-2xl">
          <DialogHeader>
            <DialogTitle className="text-xl font-bold">Create Community Group</DialogTitle>
            <DialogDescription>
          ...
          <form onSubmit={handleGroupSubmit} className="space-y-4 pt-2">
            ...
            <div className="border-border/50 flex justify-end gap-2 border-t pt-4">
              <Button type="button" variant="outline" onClick={() => setIsGroupOpen(false)}>
                Cancel
              </Button>
